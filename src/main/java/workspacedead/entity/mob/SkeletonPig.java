...
public class SkeletonPig extends SkeletonAnimal {

    public SkeletonPig(EntityType<? extends SkeletonPig> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
        this.setAggressive(true);
    }

    public SkeletonPig(Level pLevel) {
        this(ModEntityTypes.SKELETONPIG.get(), pLevel);
    }

    public static AttributeSupplier setAttributes() {
        return Animal.createMobAttributes().add(Attributes.MAX_HEALTH, 10).add(Attributes.ATTACK_DAMAGE, 2).add(Attributes.ATTACK_SPEED, 4).add(Attributes.MOVEMENT_SPEED, .2f).add(Attributes.SPAWN_REINFORCEMENTS_CHANCE).add(Attributes.FOLLOW_RANGE, 15.0D).build();
    }
    ...
    public boolean doHurtTarget(Entity pEntity) {
        ...
        if (flag) {
            ...
            this.playSound(MySounds.SKELETONPIG_ATTACKS.get(), 1.0F, this.getVoicePitch());
        }
        return flag;
    }
