...
public class MagentaGeneratorBlock extends BaseGeneratorBlock {
    // make it so it can only look NESW
    // private static final VoxelShape RENDER_SHAPE = Shapes.box(0.1, 0.1, 0.1, 0.9,
    // 0.9, 0.9);

    public MagentaGeneratorBlock(BlockBehaviour.Properties pProperties) {
        super(pProperties);
    }
    ...
    public BlockEntity newBlockEntity(BlockPos blockPos, BlockState blockState) {
        return new MagentaGeneratorBlockEntity(blockPos, blockState);
    }
