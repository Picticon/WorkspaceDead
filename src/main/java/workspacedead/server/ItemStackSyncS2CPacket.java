...
/*    public boolean handle(Supplier<NetworkEvent.Context> supplier) {
        ...
        context.enqueueWork(() -> {
            if(Minecraft.getInstance().level.getBlockEntity(pos) instanceof SaturatorBlockEntity blockEntity) {
                blockEntity.setHandler(this.itemStackHandler);
            }
            if(Minecraft.getInstance().level.getBlockEntity(pos) instanceof DesaturatorBlockEntity blockEntity) {
                blockEntity.setHandler(this.itemStackHandler);
            }
        });
        return true;
